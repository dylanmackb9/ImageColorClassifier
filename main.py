...
dataset_train = np.load(featurePath)
dataset_labels = np.load(labelPath)
...
for i in range(1000):
	sumfeature[i] = np.sum(dataset_train[i])
...
def kcross_nn(k, lr, epo, activ):
    ...
    m = int(x_train.shape[0])  # number of training examples
    kaccuracy_list = []
    
    for i in range(k):  # cross val training 

        i = tf.convert_to_tensor(i, dtype=tf.int64)
        x_val = x_train[i*m//k:i*m//k+m//k]  # finding k m/k sized portions
        y_val = y_train[i*m//k:i*m//k+m//k]  # finding k m/k sized portions
        
        x = np.vstack((x_train[0:i*m//k],x_train[(i*m//k+m//k):]))  # setting training to be everything but val
        y = np.concatenate((y_train[0:i*m//k],y_train[(i*m//k+m//k):]))
...
lr_range = [.001,.01, .1, 1, 10]
epochs = [3, 5, 10, 15, 20, 30]
...
def gridSearch():
    ...
    for activ in activation:
        for lr in lr_range:
            for ep in epochs:
                accuracy = kcross_nn(k, lr, ep, activ)
...
idealmod = nn.dNN(x_train, y_train, 'linear', .1, 15)
loss, finalaccuracy = idealmod.evaluate(x_test, y_test)
print(finalaccuracy)
